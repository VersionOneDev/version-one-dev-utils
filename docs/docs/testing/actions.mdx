...
We can test if, when and how actions from our stores are fired in our components.

<br />

## Default case

This first test example mimics a button click with helper function `userEvent` from `@testing-library/react`.
We then check that the correct action was fired with the correct value by passing `global.STORYBOOK_ACTION`
to the `expect`.
