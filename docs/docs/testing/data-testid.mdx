...
```jsx
describe("Story1", () => {
  it("renders div which wraps header and list", () => {
    render(<Story1 />);
...
Note how we target this value in our tests; the h1 will be given a `data-testid` attribute of `"todos/header"`.
...
A function returned from `TestId` can be passed as a prop to child components. This example is the same as the one
above but the `h1` element has been replaced by a `<Header/>` component and the test works the same.
...
function Header(props) {
  const testId = TestId(props);
  return <h1 {...testId()}>{props.value}</h1>;
}
