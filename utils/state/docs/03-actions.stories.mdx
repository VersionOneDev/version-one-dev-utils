<Meta title="utils/state/Actions" />
...
| meta     | Object containing the `props` and `key` passed to the action when called. See [Filtering errors with keys](/docs/utils-state-error-handling--page#filtering-by-keys) for more about keys. |
| payload  | Payload returned if action succeeds.                                                                                                                                                      |
| error    | Error thrown if action is unsuccessful.                                                                                                                                                   |

> **Note:** Store states are **immutable** so a new rather than modified state value should be returned by the reducer.
